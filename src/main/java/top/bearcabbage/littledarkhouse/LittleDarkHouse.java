    ...
    public boolean onCommand(CommandSender sender, Command command, String zhiling, String[] liebiao)
    {
        if(zhiling.equals("setxhw"))
        {
            ...
            try{
                ...
                this.getServer().getConfig().set("监狱所在的世界", this.getServer().getPlayerExact(sender.getName()).getLevel().getName());
                this.getServer().getConfig().save();
            ...
            sender.sendMessage("[小黑屋]§d小黑屋位置设定成功为 地图"+this.getServer().getLevelByName(levelname).getName()+" "+x+ " "+y+" "+z);
            return true;
        }
            ...
            if (liebiao.length == 1) {
                ...
                try {
                    ...
                    */
                    ...
                    c.setGamemode(2);
                    c.teleport(this.getServer().getLevelByName(levelname).getSafeSpawn());
                    c.teleport(xhw);
                    c.setSpawn(new Vector3(xhw.x,xhw.y,xhw.z));
